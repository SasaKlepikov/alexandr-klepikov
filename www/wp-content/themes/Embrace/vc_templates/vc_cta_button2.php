...
$css_class .= $animate;
?>
<div<?php echo $inline_css; ?> class="<?php echo esc_attr(trim($css_class)); ?>" <?php echo $animation_data;?>>
    <?php if ($link!='' && $position!='bottom') echo do_shortcode('[vc_button2 link="'.$link.'" title="'.$title.'" color="'.$color.'" button_bg_color="'.$button_bg_color.'" button_text_color="'.$button_text_color.'" icon="'.$icon.'" size="'.$size.'" style="'.$btn_style.'" el_class="vc_cta_btn"]'); ?>
<?php if ($h2!='' || $h4!=''): ?>
    <hgroup>
        <?php if ($h2!=''): ?><h2 class="wpb_heading"><?php echo $h2; ?></h2><?php endif; ?>
        <?php if ($h4!=''): ?><h6 class="wpb_heading"><?php echo $h4; ?></h6><?php endif; ?>
    </hgroup>
<?php endif; ?>
