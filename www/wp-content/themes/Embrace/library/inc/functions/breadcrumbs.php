...
class Breadcrumb_Trail {
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => __( 'You are here:',           'crum' ),
			'home'                => __( 'Home',                    'crum' ),
			'search'              => __( 'Search results for "%s"', 'crum' ),
			'error_404'           => __( '404 Not Found',           'crum' ),
			'paged'               => __( 'Page %d',                 'crum' ),
			'archives'            => __( 'Archives',                'crum' ),
			'archive_minute_hour' => __( 'g:i a',                   'crum' ),
			'archive_minute'      => __( 'Minute %d',               'crum' ),
			'archive_hour'        => __( 'g a',                     'crum' ),
			'archive_day'         => __( 'd',                       'crum' ),
			'archive_week'        => __( 'Week %d',                 'crum' ),
			'archive_month'       => __( 'F',                       'crum' ),
			'archive_year'        => __( 'Y',                       'crum' ),
			//	'edit'                => __( 'Edit',                    'crum' ), // @todo Implement edit link
		);
	...
	public function do_posts_page_items() {
		...
		$post    = get_post/*get_page*/( $post_id );
	...
	public function do_singular_items() {
		...
		if (is_singular('portfolio') || is_tax('portfolio-tag') || is_tax('portfolio-category')){

			$this->do_post_parents( reactor_option('portfolio_page_select','portfolio') );
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( __( 'Y', 'crum' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_year'], $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'crum' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_month'], $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'crum' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_day'], $post_id ) . '</a>';
